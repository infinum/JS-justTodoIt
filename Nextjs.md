...
Construct the `fallback` object with hte key/value paris where key is the URL and value is the raw response, ad pass it to the SWRConfig component. This will ensure that the client side SWR will not make an additional request to the server.
