...
Serializer.register('todo-lists', {
  id: 'uuid',
  links: () => ({
    self: '',
  }),
  topLevelMeta: (_: any, { totalCount }: IExtraData) => ({
      totalCount,

  }),
  topLevelLinks: (_: any, { totalCount }: IExtraData) => {
    return {
      self: ``,
    };
  },
...
export class JsonapiTodosController {
  constructor(
  ...
  //   passUser: true,
  // })
  @Returns(200, Object).Of(TodoList).ContentType('application/vnd.api+json')
  async fetchAll(
    ...
    const { results: data, count } = await this.todosService.fetchAll({
      user: req.user,
      relations,
      page: {
        number: pageNumber,
    ...
    return Serializer.serialize('todo-lists', data, 'default', { totalCount: count });
  }
  ...
  @Patch('/:uuid')
  @Summary('Partial or full update of a TodoList by uuid')
  // @Auth({
  //   passUser: true,
  // })
  async update(
